server 'ci.production.isobar.ru', user: 'webmaster', roles: %w{app db web}, port: 222

set :deploy_to, '/data/sites/skeleton.demo.isobar.ru/public'
set :branch, 'master'
set :symfony_env,  "prod"
set :controllers_to_clear, [] #keep app_dev.php
set :php_version, 71 #allowed: 55,56,70,71

set :default_env, { "PATH" => "/opt/php/php#{fetch(:php_version)}/bin:$PATH" }
SSHKit.config.command_map[:composer] = "php#{fetch(:php_version)} $(which composer)"

before 'deploy:starting', 'deploy:phpinfo'
